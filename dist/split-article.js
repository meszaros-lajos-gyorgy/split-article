...
var isReady = function () { return ['interactive', 'complete'].includes(document.readyState); };
...
var generateMeasureText = compose(join(''), repeat('a'));

var getMeasuredWidth = function (source, width) {
...
var splitToWords = function (child) {
  var elements = child.innerHTML.match(/(?:<[^>]+>|[^\r\n\t <]+)/g);
  var tags = [];
...
var getWordWidths = function (child, paragraph) {
  ...
  return {
    ...
    wordWidths: map(function (node) { return node.scrollWidth; }, Array.from(child.children))
  }
};
...
var SplitArticle = function SplitArticle (rawConfig) {
  ...
  onReady(function () {
    ...
    */
    ...
    this$1.children = Array.from(this$1.config.source.children);
    this$1.config.source.style = 'height:0;position:absolute;overflow:hidden';

    this$1.measuredWidth = getMeasuredWidth(this$1.config.source, this$1.config.width);
    this$1.config.source.style.width = this$1.measuredWidth + 'px';

    var paragraphs = map(splitToWords, this$1.children);

    // ------------

    var ref = getWordWidths(this$1.children[0], paragraphs[0]);
